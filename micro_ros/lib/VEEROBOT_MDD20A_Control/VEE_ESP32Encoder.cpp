...
static const char* TAG = "VEE_ESP32Encoder";
...
//static VEE_ESP32Encoder *gpio2enc[48];
//
//
enum puType VEE_ESP32Encoder::useInternalWeakPullResistors=DOWN;
uint32_t VEE_ESP32Encoder::isrServiceCpuCore = ISR_CORE_USE_DEFAULT;
VEE_ESP32Encoder *VEE_ESP32Encoder::encoders[MAX_ESP32_ENCODERS] = { NULL, };

bool VEE_ESP32Encoder::attachedInterrupt=false;

VEE_ESP32Encoder::VEE_ESP32Encoder(bool always_interrupt_, enc_isr_cb_t enc_isr_cb, void* enc_isr_cb_data):
...
VEE_ESP32Encoder::~VEE_ESP32Encoder() {}
...
static void esp32encoder_pcnt_intr_handler(void *arg) {
  VEE_ESP32Encoder * esp32enc = static_cast<VEE_ESP32Encoder *>(arg);
...
void VEE_ESP32Encoder::detach(){
  pcnt_counter_pause(unit);
  pcnt_isr_handler_remove(this->r_enc_config.unit);
  VEE_ESP32Encoder::encoders[unit]=NULL;
}

void VEE_ESP32Encoder::detatch(){
  this->detach();
}
...
void VEE_ESP32Encoder::attach(int a, int b, enum encType et) {
  if (attached) {
  ...
  for (; index < MAX_ESP32_ENCODERS; index++) {
    if (VEE_ESP32Encoder::encoders[index] == NULL) {
      encoders[index] = this;
      break;
    }
  }
...
void VEE_ESP32Encoder::attachHalfQuad(int aPintNumber, int bPinNumber) {
  attach(aPintNumber, bPinNumber, half);

}
void VEE_ESP32Encoder::attachSingleEdge(int aPintNumber, int bPinNumber) {
  attach(aPintNumber, bPinNumber, single);
}
void VEE_ESP32Encoder::attachFullQuad(int aPintNumber, int bPinNumber) {
  attach(aPintNumber, bPinNumber, full);
}

void VEE_ESP32Encoder::setCount(int64_t value) {
  _ENTER_CRITICAL();
  count = value - getCountRaw();
  _EXIT_CRITICAL();
}

int64_t VEE_ESP32Encoder::getCountRaw() {
  int16_t c;
...
int64_t VEE_ESP32Encoder::getCount() {
  _ENTER_CRITICAL();
...
int64_t VEE_ESP32Encoder::clearCount() {
  _ENTER_CRITICAL();
  count = 0;
  _EXIT_CRITICAL();
...
int64_t VEE_ESP32Encoder::pauseCount() {
  return pcnt_counter_pause(unit);
}

int64_t VEE_ESP32Encoder::resumeCount() {
  return pcnt_counter_resume(unit);
}

int64_t VEE_ESP32Encoder::getRPM(int counts_per_rev){
  long encoder_ticks = getCount();
  unsigned long current_time = millis();
  unsigned long dt = current_time - prev_update_time;
  double dtm = (double)dt / 60000;    //convert the time from milliseconds to minutes
  double delta_ticks = encoder_ticks - prev_encoder_ticks;
  prev_update_time = current_time;    //calculate wheel's speed (RPM)
  prev_encoder_ticks = encoder_ticks;
  return (delta_ticks / counts_per_rev) / dtm;
}

void VEE_ESP32Encoder::setFilter(uint16_t value) {
