...
public:
  /**
   * @brief Construct a new VEE_ESP32Encoder object
   *
   ...
   *  the VEE_ESP32Encoder instance as an argument, no effect if always_interrupt is
   *  false
   */
  VEE_ESP32Encoder(bool always_interrupt=false, enc_isr_cb_t enc_isr_cb=nullptr, void* enc_isr_cb_data=nullptr);
  ~VEE_ESP32Encoder();
  ...
  int64_t pauseCount();
  int64_t resumeCount();
  int64_t getRPM(int counts_per_rev);
  void detach();
  [[deprecated("Replaced by detach")]] void detatch();
  bool isAttached(){return attached;}
  void setCount(int64_t value);
  void setFilter(uint16_t value);
  static VEE_ESP32Encoder *encoders[MAX_ESP32_ENCODERS];
