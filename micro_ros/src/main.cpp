...
const float RPM_MIN = -55.0;   // Minimum RPM value
const float RPM_MAX = 55.0; // Maximum RPM value
...
//   ##################################################################
// Motor objects
VEE_CytronMotorDriver frontLeftMotor(M1_PWM, M1_DIR);
VEE_CytronMotorDriver frontRightMotor(M2_PWM, M2_DIR);
VEE_CytronMotorDriver backRightMotor(M3_PWM, M3_DIR);
VEE_CytronMotorDriver backLeftMotor(M4_PWM, M4_DIR);

enum Motor { FRONTLEFT, FRONTRIGHT, BACKRIGHT, BACKLEFT };

void setMotorSpeed(Motor motor, int spd) {
  VEE_CytronMotorDriver* targetMotor;
  // Select the correct motor based on the input
  switch (motor) {
    case FRONTLEFT:
      targetMotor = &frontLeftMotor;
      break;
    case FRONTRIGHT:
      targetMotor = &frontRightMotor;
      break;
    case BACKRIGHT:
      targetMotor = &backRightMotor;
      break;
    case BACKLEFT:
      targetMotor = &backLeftMotor;
      break;
  }

  // Set motor speed and direction
  if (spd == 0) {
    targetMotor->setSpeed(0); // Stopping the motor
  } else {
    targetMotor->setSpeed(abs(spd)); // Setting speed
    if (spd > 0) {
      targetMotor->run(FORWARD);
    } else {
      targetMotor->run(BACKWARD);
    }
  }
}
...
void setMotorSpeeds(int frontLeftSpeed, int frontRightSpeed, int backRightSpeed, int backLeftSpeed) {
  ...
  setMotorSpeed(BACKRIGHT, -backRightSpeed); // Invert speed if necessary
  setMotorSpeed(BACKLEFT, -backLeftSpeed); // Invert speed if necessary
}
